...
def add_subtask(assignment_id):
    ...
    # Validasi duplikat nama subtask
    existing = Subtask.query.filter_by(assignment_id=assignment_id, name=name).first()
    if existing:
        return jsonify({'error': 'Subtask dengan nama yang sama sudah ada'}), 400
...
@routes.route('/subtasks/<int:assignment_id>', methods=['DELETE'])
def delete_all_subtasks(assignment_id):
    user_id = session.get('user_id')
    if not user_id:
        return jsonify({'error': 'Unauthorized'}), 401
    ...
    Subtask.query.filter_by(assignment_id=assignment_id).delete()
    db.session.commit()
    return jsonify({'message': 'Semua subtask berhasil dihapus'})
